...
import { ThriveMcp, ClientOptions } from 'thrive-mcp';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: ThriveMcp)`
    : `const run: (${functionSource.client}: ThriveMcp) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
