...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['THRIVE_MCP_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['THRIVE_MCP_LOG'] or 'warn' if it isn't set.
   */
...
export class ThriveMcp {
   ...
   * @param {Environment} [opts.environment=Leadsnap] - Specifies the environment URL to use for the API.
   ...
   */
  constructor({
    baseURL = readEnv('THRIVE_MCP_BASE_URL'),
    bearerToken = readEnv('THRIVE_MCP_BEARER_TOKEN'),
    ...opts
  }: ClientOptions = {}) {
    if (bearerToken === undefined) {
      throw new Errors.ThriveMcpError(
        "The THRIVE_MCP_BEARER_TOKEN environment variable is missing or empty; either provide it, or instantiate the ThriveMcp client with an bearerToken option, like new ThriveMcp({ bearerToken: 'My Bearer Token' }).",
      );
    }
    ...
    const options: ClientOptions = {
      ...
      ...opts,
      baseURL,
      environment: opts.environment ?? 'Leadsnap',
    };
    ...
    this.baseURL = options.baseURL || environments[options.environment || 'Leadsnap'];
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('THRIVE_MCP_LOG'), "process.env['THRIVE_MCP_LOG']", this) ??
      defaultLogLevel;
    ...
    const customHeadersEnv = readEnv('THRIVE_MCP_CUSTOM_HEADERS');
    if (customHeadersEnv) {
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== environments[this._options.environment || 'Leadsnap'];
  }
  ...
  static ThriveMcp = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static ThriveMcpError = Errors.ThriveMcpError;
