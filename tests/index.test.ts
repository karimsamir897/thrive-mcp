...
import { APIPromise } from 'thrive-mcp/core/api-promise';

import util from 'node:util';
import ThriveMcp from 'thrive-mcp';
import { APIUserAbortError } from 'thrive-mcp';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['THRIVE_MCP_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: ThriveMcp) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['THRIVE_MCP_LOG'] = 'debug';
      const client = new ThriveMcp({ logger: logger, bearerToken: 'My Bearer Token' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['THRIVE_MCP_LOG'] = 'not a log level';
      const client = new ThriveMcp({ logger: logger, bearerToken: 'My Bearer Token' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'THRIVE_MCP_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['THRIVE_MCP_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new ThriveMcp({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
    ...
    test('empty env variable', () => {
      ...
      const client = new ThriveMcp({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://app.crmthrive.com');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['THRIVE_MCP_BASE_URL'] = 'http://localhost:5000/env';
      const client = new ThriveMcp({ bearerToken: 'My Bearer Token' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new ThriveMcp({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new ThriveMcp({ bearerToken: 'My Bearer Token' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['THRIVE_MCP_BEARER_TOKEN'] = 'My Bearer Token';
    const client = new ThriveMcp();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['THRIVE_MCP_BEARER_TOKEN'] = 'another My Bearer Token';
    const client = new ThriveMcp({ bearerToken: 'My Bearer Token' });
    expect(client.bearerToken).toBe('My Bearer Token');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new ThriveMcp({ bearerToken: 'My Bearer Token', fetch: testFetch });
